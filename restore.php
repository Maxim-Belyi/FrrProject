...
{
	$strLog = '';
	if (LoadFile("https://" . $util_host . "/backup.php?license=" . md5(trim($_REQUEST['license_key'])) . "&lang=" . LANG . "&action=get_info&is_restore=y", $file = $_SERVER['DOCUMENT_ROOT'] . '/file_list.xml') && ($str = file_get_contents($file)))
	{
...
if ($Step == 2 && !$bSelectDumpStep)
{
	...
	{
		$strLog = '';
		$downloadUrl = 'https://' . $util_host . '/backup.php?license=' . md5(trim($_REQUEST['license_key'])) . '&lang=' . LANG . '&action=read_file&file_name=' . urlencode($_REQUEST['bitrixcloud_backup']) . '&check_word=' . CTar::getCheckword($_REQUEST['EncryptKey']).'&is_restore=y';
